    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_300px]">
        <div className="grid min-w-0 gap-4 xl:grid-cols-2">
          <LineChart title={s.positionGraph} series={[{ label: s.position, points: shown.map((p) => ({ x: p.time, y: p.x })) }]} xLabel={`${s.time} (${s.units.s})`} yLabel={`${s.position} (${s.units.m})`} xDomain={[0, DURATION]} yDomain={[xMin, xMax]} width={480} height={260} />
          <LineChart
            title={s.velocityGraph}
            series={[{ label: s.velocity, points: shown.map((p) => ({ x: p.time, y: p.v })), color: "var(--color-warn)" }]}
            xLabel={`${s.time} (${s.units.s})`}
            yLabel={`${s.velocity} (${s.units.mps})`}
            xDomain={[0, DURATION]}
        ...
        <div className="space-y-4">
          <SimSlider label={s.v0} value={v0} min={-5} max={10} step={0.5} unit={s.units.mps} onChange={(v) => { setV0(v); setT(0); setPlaying(false); }} />
          <SimSlider label={s.acceleration} value={a} min={-3} max={3} step={0.1} unit={s.units.mps2} format={(v) => num(v, 1)} onChange={(v) => { setA(v); setT(0); setPlaying(false); }} />
          <div className="grid grid-cols-3 gap-2">
            <Readout label={s.time} value={`${num(t, 1)} ${s.units.s}`} />
            <Readout label={s.position} value={`${num(now.x, 1)} ${s.units.m}`} />
            <Readout label={s.velocity} value={`${num(now.v, 1)} ${s.units.mps}`} />
          </div>
