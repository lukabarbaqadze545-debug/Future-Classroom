...
export function ProjectileSim({ report }: { report: TaskReporter }) {
  ...
  const update = (next: Partial<{ angle: number; speed: number; height: number; planet: keyof typeof GRAVITY }>) => {
    ...
    setProgress(1);
    report("projectile_target", {
      summary: `${v.angle}°, ${v.speed} m/s, ${s.planets[v.planet]}${v.height ? `, h = ${v.height} m` : ""}`,
      value: { angle: v.angle, speed: v.speed, gravity: GRAVITY[v.planet], height: v.height },
    });
  };
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[1fr_300px]">
      <div className="min-w-0">
        <LineChart
          title={`${s.range}: ${fixed(shot.range, 1)} m`}
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          <Readout label={s.range} value={`${fixed(shot.range, 1)} m`} />
          <Readout label={s.maxHeight} value={`${fixed(shot.maxHeight, 1)} m`} />
          <Readout label={s.flightTime} value={`${fixed(shot.flightTime, 2)} s`} />
        </div>
      </div>
      <div className="space-y-4">
        <SimSlider label={s.angle} value={angle} min={0} max={90} step={1} format={(v) => `${v}°`} onChange={(v) => update({ angle: v })} />
        <SimSlider label={s.speed} value={speed} min={1} max={40} step={0.1} unit="m/s" onChange={(v) => update({ speed: Math.round(v * 10) / 10 })} />
        <SimSlider label={s.height} value={height} min={0} max={30} step={1} unit="m" onChange={(v) => update({ height: v })} />
        <div>
          <p className="mb-1.5 text-sm font-medium">{s.gravity}</p>
          <Segmented label={s.gravity} value={planet} options={(["earth", "moon", "mars"] as const).map((id) => ({ id, label: `${s.planets[id]} (${GRAVITY[id]})` }))} onChange={(v) => update({ planet: v })} />
        </div>
...
export const PROJECTILE_DEFAULT = { summary: "45°, 15 m/s", value: { angle: 45, speed: 15, gravity: GRAVITY.earth, height: 0 } };
